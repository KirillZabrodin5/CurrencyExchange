...
public class JdbcExchangeRateDao implements CrudDao<ExchangeRate, String> {
    ...
    public ExchangeRate findByCode(String baseCode, String targetCode) {
        ...
        ReceivedRate rate = new ReceivedRate(
                jdbcCurrencyDao.findByCode(baseCode).getCode(),
                jdbcCurrencyDao.findByCode(targetCode).getCode()
        );

        ExchangeRate rates = new ExchangeRate(idExRate,
                findByCode(baseCode),
    ...
    private int getIdExRate(String baseCode, String targetCode) {
        ...
        try (
            ...
            stmt.setInt(1, findByCode(baseCode).getId());
            stmt.setInt(2, findByCode(targetCode).getId());
    ...
    public void save(ExchangeRate exchangeRate) {
        ...
        try (
            ...
            JdbcCurrencyDao jdbcCurrencyDao = new JdbcCurrencyDao();
            int idStart = jdbcCurrencyDao
                    .findById(exchangeRate.getBaseCurrency().getId())
                    .orElse(null)
                    .getId();
            int idEnd = jdbcCurrencyDao
                    .findById(exchangeRate.getTargetCurrency().getId())
                    .orElse(null)
                    .getId();
    ...
    public void update(ExchangeRate exchangeRate/*String codeStartCurrency, String codeEndCurrency,
                                    int rate*/) {
        final String sql = """
